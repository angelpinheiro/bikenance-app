...
sealed class BikeSetupScreenState {
    ...
    data class SetupInProgress(
        ...
        val details: SetupDetails = SetupDetails()
...
data class SetupDetails(
    ...
    val lastMaintenances: Map<ComponentCategory, Float> = ComponentCategory.values().toList().associateWith { 0f },
...
class BikeSetupViewModel @Inject constructor(
            ...
            is BikeSetupScreenState.SetupInProgress -> {
                ...
                currentState.copy(
                    details = details.copy(
                        lastComponentMaintenances = getComponentsForSetup(
                    details
                ).associateWith { 0f }
                    )
                )
    ...
    private fun getComponentsForSetup(details: SetupDetails): List<ComponentType> {
        var componentTypes = MaintenanceConfiguration.forBikeType(details.selectedBikeType).componentTypes
